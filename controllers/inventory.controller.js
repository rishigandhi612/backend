...
const validateInventoryData = (data) => {
  ...
  if (
    data.status &&
    !["damaged", "available", "reserved", "sold"].includes(data.status)
  ) {
  ...
  if (data.netWeight && typeof data.netWeight !== "number")
    errors.push("Net weight must be a number");
  if (data.grossWeight && typeof data.grossWeight !== "number")
    errors.push("Gross weight must be a number");
  if (data.micron && typeof data.micron !== "number")
    errors.push("Micron must be a number");
  if (data.mtr && typeof data.mtr !== "number")
    errors.push("Meter must be a number");
...
const generateSequentialRollId = async (netWeight, width) => {
  ...
  const widthStr = Math.round(width * 100)
    .toString()
    .padStart(4, "0")
    .slice(0, 4);
  const weightStr = Math.round(netWeight * 100)
    .toString()
    .padStart(4, "0")
    .slice(0, 4);
  ...
  if (
    latestInventory &&
    latestInventory.rollId.length === basePattern.length + 2
  ) {
  ...
  try {
    ...
    if (existingItem.status === "sold") {
      return res.status(400).json({
        success: false,
        message:
          "Cannot delete sold inventory items. Consider changing status instead.",
      });
    }
...
const getInventoryByStatus = async (req, res) => {
  ...
  if (!["damaged", "available", "reserved", "sold"].includes(status)) {
    return res.status(400).json({
      success: false,
      message:
        "Invalid status. Must be one of: damaged, available, reserved, sold",
    });
  }
